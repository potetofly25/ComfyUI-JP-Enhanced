...
CodeGraphを利用できる環境で、行う作業がグラフ探索に適している場合に優先利用し、不適合、利用不能、情報不足の場合は通常の検索・読取・実行確認へ安全にフォールバックする。

詳細は、`.project-knowledge/指示セット/10_CodeGraph利用ガイド.md`を参照する。

## 知識の扱い

ルート`.project-knowledge/`は、導入先固有の仕様、用語、構成、判断、制約、検証知識、継続利用するプロジェクト指示情報を置く任意のOKF v0.2バンドルである。

- 存在しないことを警告や作業阻害にしない。
- 存在する場合は目的の詳細より先に関連コンセプトを選ぶ。
- 知識内では高い参照優先度を持つが、ユーザー指示、正規指示、操作権限、安全制約を上書きしない。
- 実装、設定、テスト、正本と矛盾する場合は、更新日、出典、状態、実際の動作を確認する。
- 秘密情報、認証情報、本番接続情報、その場限りのログを保存しない。


## ワークフローの命名と扱い

- `F`：ベースフロー。コピーして実用するためのひな形。
- `T`：ツール用フロー。
- `W`：個人用フロー。ユーザーから指示がない限り、内容確認やフロー編集を行わない。
- 命名形式は `記号00_内容名称` とする。記号は `F`・`T`・`W`、`00` は2桁の番号を表す。
- 内容名称にはモデル名などではなく、役割やできることを端的に表す名称を付ける（例：`F01_画像生成`、`T01_画像拡大`）。

ワークフローの作成・配置変更では、`.project-knowledge/指示セット/20_ワークフロー作成ルール.md`に従う。

## 独自機能の配置

- このリポジトリで追加・改修する独自機能を`comfy_extras`に置く場合は、`comfy_extras/jp_enhanced/<機能名>/`へ配置する。
- 各機能のノード実装、Web拡張、辞書等は、その機能のサブフォルダ内にまとめる。異なる機能は別フォルダに分離する。
- 対応する独自機能のテストは`tests-unit/comfy_extras_test/jp_enhanced/<機能名>/`に配置し、実装側と機能名をそろえる。複数機能で実際に共用するテスト補助だけを`jp_enhanced/_support/`へ置く。
- 現在の区分は`prompt`（独自プロンプトノード）、`node_tooltips`（ノード説明の日本語化）、`error_support`（エラー支援）、`startup_sidebar`（起動時の左サイドバー表示）。
- 読み込み登録とテストの参照も同時に更新し、既存フローのノードID・入出力互換性を維持する。移動前の場所へ重複実装を残さない。
- 上流標準の`comfy_extras`機能は、この整理のためだけに移動しない。
